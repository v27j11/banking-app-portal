...
public class AccountController {
    ...
    /**
     * Paginated transaction history, newest first. Prefer this over
     * {@code /transactions} for accounts with a long history.
     */
    @GetMapping("/transactions/page")
    public ResponseEntity<Page<TransactionDTO>> getTransactionsPage(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size) {
        val pageable = PageRequest.of(page, Math.min(size, 100));
        val transactions = transactionService
                .getTransactionsByAccountNumber(LoggedinUser.getAccountNumber(), pageable);
