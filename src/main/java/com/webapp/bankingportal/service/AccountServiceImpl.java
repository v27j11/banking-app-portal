...
@Autowired
    ...
    /**
     * Publishes a {@link TransactionEvent} for asynchronous, off-request-thread
     * processing (e.g. customer notifications). Published via Spring's
     * application event mechanism rather than sent to Kafka directly so that
     * {@link com.webapp.bankingportal.messaging.TransactionEventProducer} can
     * defer the actual publish until AFTER this database transaction commits -
     * a rolled-back transaction (e.g. insufficient balance) must never result
     * in a notification being sent.
     */
    private void publishTransactionEvent(Transaction transaction) {
        val event = new TransactionEvent(
                transaction.getId(),
