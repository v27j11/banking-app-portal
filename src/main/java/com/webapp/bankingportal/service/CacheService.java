...
public interface CacheService {
    ...
    /**
     * Atomically stores the value only if the key is not already present
     * (Redis {@code SETNX}). Used to acquire a short-lived lock, e.g. to
     * prevent two concurrent identical requests from both being processed.
     *
     * @return {@code true} if the value was stored (lock acquired),
     *         {@code false} if the key already existed (lock held by someone
     *         else).
     */
    boolean putIfAbsent(CacheKeyType cacheKeyType, Object value, long ttlSeconds, String... keyArguments);
