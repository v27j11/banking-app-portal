...
public class CacheServiceImpl implements CacheService {
    ...
    @Override
    public boolean putIfAbsent(CacheKeyType cacheKeyType, Object value, long ttlSeconds, String... keyArguments) {
        String key = acquireKey(cacheKeyType, keyArguments);

        try {
            Boolean acquired = redisTemplate.opsForValue()
                    .setIfAbsent(key, value, ttlSeconds, TimeUnit.SECONDS);
            log.debug("Cache putIfAbsent: key={}, acquired={}", key, acquired);
            return Boolean.TRUE.equals(acquired);
        } catch (Exception e) {
            log.error("Error storing value in cache (putIfAbsent): {}", e.getMessage());
            throw new RuntimeException("Failed to store value in cache", e);
        }
    }
