...
class CacheServiceTests {
    ...
    @Test
    void testPutIfAbsent_WhenKeyAbsent_StoresValueAndReturnsTrue() {
        String userId = "user123";
        String endpoint = "/api/account/deposit";
        String payloadHash = "payload123";
        String expectedKey = "IDPT:user123:/api/account/deposit:payload123";
        ...
        when(redisTemplate.opsForValue()).thenReturn(valueOperations);
        when(valueOperations.setIfAbsent(expectedKey, value, 30, java.util.concurrent.TimeUnit.SECONDS))
                .thenReturn(true);

        boolean acquired = cacheService.putIfAbsent(CacheKeyType.IDEMPOTENCY, value, 30, userId, endpoint,
                payloadHash);

        assertTrue(acquired);
        verify(valueOperations).setIfAbsent(expectedKey, value, 30, java.util.concurrent.TimeUnit.SECONDS);
    }

    @Test
    void testPutIfAbsent_WhenKeyPresent_ReturnsFalse() {
        String userId = "user123";
        String endpoint = "/api/account/deposit";
        String payloadHash = "payload123";
        String expectedKey = "IDPT:user123:/api/account/deposit:payload123";
